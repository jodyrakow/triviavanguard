 ...
 * Copies Question text/flavor/answer from Questions table,
 * and pulls Question-level attachments from Images/Audio tables using the
 * linked-record fields on the Questions row.
 *
 * For Tiebreakers: Uses "Tiebreaker answer" and "Tiebreaker number" instead of regular answer.
 *
...
const DEST_ON_SQ = {
  ...
  Q_FLAVOR:  "Flavor text",
  Q_ANSWER:  "Answer",
  TB_NUMBER: "Tiebreaker number",
...
function get(cell) { return cell ?? null; }
function toAttach(cellValue) {
  if (!Array.isArray(cellValue)) return [];
  return cellValue.filter(a => a && a.url).map(a => ({ url: a.url, filename: a.filename || undefined }));
}
 ...
 */
async function fetchChildAttachmentsByIds({ table, idsInOrder, fileField, maybeOrderField }) {
  ...
  for (const id of idsInOrder) {
    ...
    rows.push({ files, order: typeof orderVal === "number" ? orderVal : undefined });
  }
...
const isTiebreaker = questionType && String(questionType).toLowerCase() === "tiebreaker";
...
if (questionId) {
  ...
  if (question) {
    // Copy question text and flavor (same for all questions)
    if (hasField(SQ, DEST_ON_SQ.Q_TEXT))
      dest[DEST_ON_SQ.Q_TEXT] = get(question.getCellValue(QUESTION_FIELDS_ON_QUESTIONS.TEXT));
    if (hasField(SQ, DEST_ON_SQ.Q_FLAVOR))
      dest[DEST_ON_SQ.Q_FLAVOR] = get(question.getCellValue(QUESTION_FIELDS_ON_QUESTIONS.FLAVOR));

    // Copy answer - use tiebreaker fields if applicable
    if (hasField(SQ, DEST_ON_SQ.Q_ANSWER)) {
      if (isTiebreaker) {
        dest[DEST_ON_SQ.Q_ANSWER] = get(question.getCellValue(QUESTION_FIELDS_ON_QUESTIONS.TB_ANSWER));
      } else {
        dest[DEST_ON_SQ.Q_ANSWER] = get(question.getCellValue(QUESTION_FIELDS_ON_QUESTIONS.ANSWER));
      }
    }

    // Copy tiebreaker number if this is a tiebreaker
    if (isTiebreaker && hasField(SQ, DEST_ON_SQ.TB_NUMBER)) {
      dest[DEST_ON_SQ.TB_NUMBER] = get(question.getCellValue(QUESTION_FIELDS_ON_QUESTIONS.TB_NUMBER));
    }

    // Get linked media IDs
    const imgLinks = question.getCellValue(QUESTION_FIELDS_ON_QUESTIONS.IMAGES_LINK) || [];
    const audLinks = question.getCellValue(QUESTION_FIELDS_ON_QUESTIONS.AUDIO_LINK)  || [];
    ...
    if (hasField(SQ, DEST_ON_SQ.Q_FLAVOR)) dest[DEST_ON_SQ.Q_FLAVOR] = null;
