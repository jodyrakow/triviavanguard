...
if (mode === "existing") {
  ...
  const allShows = await showsTable.selectRecordsAsync({
    fields: [SHOW_FIELDS.DATE, SHOW_FIELDS.DESCRIPTION, SHOW_FIELDS.TEMPLATE],
    sorts: [{ field: SHOW_FIELDS.DATE, direction: "desc" }],
  });
  // === Filter to shows on or after today ===
  const today = new Date();
  today.setHours(0, 0, 0, 0); // normalize to midnight

  const futureShows = allShows.records.filter((show) => {
    const showDate = show.getCellValue(SHOW_FIELDS.DATE);
  ...
  if (futureShows.length === 0) {
    output.markdown("❌ No upcoming Shows found.");
    return;
  }

  // Limit to most recent 20 (still sorted desc)
  const MAX_SHOWS = 20;
  const recentShows = futureShows.slice(0, MAX_SHOWS);
  ...
  output.markdown("## 🧪 Template debug");
  output.markdown(`Template: **${selectedTemplate.name}**`);

  output.markdown(
    `NUM_ROUNDS raw: \`${JSON.stringify(selectedTemplate.getCellValue(ST_FIELDS.NUM_ROUNDS))}\``,
  );
  output.markdown(
    `V2 raw: \`${JSON.stringify(selectedTemplate.getCellValue(ST_FIELDS.V2))}\``,
  );
  output.markdown(
    `S2 raw: \`${JSON.stringify(selectedTemplate.getCellValue(ST_FIELDS.S2))}\``,
  );
  output.markdown(
    `A2 raw: \`${JSON.stringify(selectedTemplate.getCellValue(ST_FIELDS.A2))}\``,
  );

  output.markdown(`numRounds parsed: **${numRounds}**`);
  output.markdown(`countsR1 parsed: **${JSON.stringify(countsR1)}**`);
  output.markdown(`countsR2 parsed: **${JSON.stringify(countsR2)}**`);
  output.markdown(`perType parsed: **${JSON.stringify(perType)}**`);
